...
@register_dataset
class SemanticKITTI_polarmix(data.Dataset):
    def __init__(self, config, data_path, imageset='train', num_vote=1):
        ...
        if imageset == 'train':
            for i_folder in split:
                self.im_idx_ins += absoluteFilePaths('/'.join([data_path, str(i_folder).zfill(2), 'ins']), num_vote)

        seg_num_per_class = config['dataset_params']['seg_labelweights']
        seg_labelweights = seg_num_per_class / np.sum(seg_num_per_class)
        self.seg_labelweights = np.power(np.amax(seg_labelweights) / seg_labelweights, 1 / 3.0)
        if 'weak' in config['dataset_params'].keys():
            self.weak = config['dataset_params']['weak']
        else:
            self.weak = False
    ...
    def __getitem__(self, index):
        ...
        if self.imageset == 'train':
            ins_id = int(np.random.rand() * len(self.im_idx_ins))
            raw_data_ins = np.fromfile(self.im_idx_ins[ins_id]).reshape((-1, 3))
            points = np.concatenate((points, raw_data_ins), axis=0)
            raw_data = np.concatenate((raw_data, np.zeros([raw_data_ins.shape[0], 4])), axis=0)

        if self.imageset == 'test':
            annotated_data = np.expand_dims(np.zeros_like(raw_data[:, 0], dtype=int), axis=1)
            instance_label = np.expand_dims(np.zeros_like(raw_data[:, 0], dtype=int), axis=1)
            annotated_data_o = annotated_data.copy()
        else:
            if self.weak:
                label_dir = 'labels_w_' + str(self.weak)
            else:
                label_dir = 'labels'
            annotated_data = np.fromfile(self.im_idx[index].replace('velodyne', label_dir)[:-3] + 'label',
            ...
            if self.imageset == 'train':
                annotated_data_ins = np.fromfile(self.im_idx_ins[ins_id].replace('ins', 'ins_labels')[:-3] + 'label',
                                         dtype=np.uint32).reshape((-1, 1))
                instance_label_ins = annotated_data_ins >> 16
                annotated_data_ins = annotated_data_ins & 0xFFFF  # delete high 16 digits binary
                annotated_data_o_ins = annotated_data_ins.copy()
                if annotated_data_ins.shape[0]:
                    annotated_data_ins = np.vectorize(self.learning_map.__getitem__)(annotated_data_ins)
                instance_label = np.concatenate((instance_label, instance_label_ins), axis=0)
                annotated_data = np.concatenate((annotated_data, annotated_data_ins), axis=0)
                annotated_data_o = np.concatenate((annotated_data_o, annotated_data_o_ins), axis=0)
        ...
        data_dict['labels_o'] = annotated_data_o.astype(np.uint8)
...
class nuScenes(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        annotated_data_o = sem_label.copy()
...
class nuScenes_6(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # get image feature
        image, cam_sample_token = self.loadImage(index)
