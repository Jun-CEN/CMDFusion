...
from network.voxel_fea_generator import voxel_3d_generator, voxelization, voxel_3d_generator_g
...
class point_encoder(nn.Module):
    ...
    def forward(self, features, data_dict):
        output, inv = self.downsample(data_dict['coors_{}'.format(self.modality)], features)
        ...
        v_feat = torch_scatter.scatter_mean(
            self.layer_out(output[data_dict['coors_inv_{}'.format(self.modality)]]),
            data_dict['scale_{}'.format(self.scale)]['coors_inv'],
            dim=0
        )
        data_dict['coors_{}'.format(self.modality)] = data_dict['scale_{}'.format(self.scale)]['coors']
        data_dict['coors_inv_{}'.format(self.modality)] = data_dict['scale_{}'.format(self.scale)]['coors_inv']
        data_dict['full_coors_{}'.format(self.modality)] = data_dict['scale_{}'.format(self.scale)]['full_coors']

        return v_feat


class SPVBlock(nn.Module):
    def __init__(self, in_channels, out_channels, indice_key, scale, last_scale, spatial_shape, modality):
        ...
        self.p_enc = point_encoder(in_channels, out_channels, scale, modality)
        self.modality = modality
    ...
    def forward(self, data_dict):
        ...
        # voxel encoder
        v_fea = self.v_enc(data_dict['sparse_tensor_{}'.format(self.modality)])
        data_dict['layer_{}_{}'.format(self.layer_id, self.modality)] = {}
        data_dict['layer_{}_{}'.format(self.layer_id, self.modality)]['pts_feat'] = v_fea.features
        data_dict['layer_{}_{}'.format(self.layer_id, self.modality)]['full_coors'] = data_dict['full_coors_{}'.format(self.modality)]
        v_fea_inv = torch_scatter.scatter_mean(v_fea.features[coors_inv_last], coors_inv, dim=0)

        # point encoder
        p_fea = self.p_enc(
            features=data_dict['sparse_tensor_{}'.format(self.modality)].features+v_fea.features,
            data_dict=data_dict
        )
        data_dict['layer_{}_{}'.format(self.layer_id, self.modality)]['pts_feat_f'] = p_fea[coors_inv]

        # fusion and pooling
        data_dict['sparse_tensor_{}'.format(self.modality)] = spconv.SparseConvTensor(
...
class get_model(LightningBaseModel):
    def __init__(self, config):
        ...
        # input processing
        self.voxel_3d_generator = voxel_3d_generator_g(
