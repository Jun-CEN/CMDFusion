...
def parse_config():
    ...
    parser.add_argument('--log_folder', type=str, default='/mnt/data_nas/jcenaa/dataset_av/semantickitti/logs', help='log location')
    ...
    parser.add_argument('--dual', type=str, default=None, help='load checkpoint')
    # debug
...
if __name__ == '__main__':
    ...
    if configs.checkpoint is not None:
        ...
        ckpt = torch.load(configs.checkpoint)
    ...
    #         my_model = my_model.load_from_checkpoint(configs.checkpoint, strict = False)
    
    if configs.dual is not None:
        state_dict = torch.load(configs.dual.path)
