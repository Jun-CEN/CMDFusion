...
def validate_submission(nusc: NuScenes,
    ...
    for sample_token in tqdm(sample_tokens, disable=not verbose):
        ...
        # print(np.unique(lidarseg_pred, return_counts=True)[0])
        # print(np.unique(lidarseg_pred, return_counts=True)[1])

        if 0 in np.unique(lidarseg_pred, return_counts=True)[0]:
            print(sd_token)
            print(idx+1)
        idx += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataroot', type=str, default='/mnt/data_nas/jcenaa/dataset_av/nuscenes/2.dataset/nuscenes/',
