...
def main():
    ...
    ind_uncertainties_lidar = uncertainty[labels_raw == 0]
    ood_uncertainties_lidar = uncertainty[labels_raw == 1]

    result_file = "/mnt/data_nas/jcenaa/code_av/2DPASS/ckpt/fusion_maxlogit_RCF_2gpu_img.npz"
    results = np.load(result_file, allow_pickle=True)
    predictions = results['predictions'].squeeze()  # (N1,)
    labels = results['labels'].squeeze()  # (N2,)
    uncertainty = results['uncertainty'].squeeze()  # (N1,)
    ...
    ind_uncertainties_img = -uncertainty[labels_raw == 0]
    ood_uncertainties_img = -uncertainty[labels_raw == 1]

    ind_uncertainties_fuse = ind_uncertainties_lidar
    ood_uncertainties_fuse = ood_uncertainties_lidar
    ind_uncertainties_fuse = ind_uncertainties_img
    ood_uncertainties_fuse = ood_uncertainties_img
    # ind_uncertainties_fuse = ind_uncertainties_img + ind_uncertainties_lidar
    # ood_uncertainties_fuse = ood_uncertainties_img + ood_uncertainties_lidar
    # ind_uncertainties_fuse = np.where(ind_uncertainties_img > ind_uncertainties_lidar, ind_uncertainties_img, ind_uncertainties_lidar)
    # ood_uncertainties_fuse = np.where(ood_uncertainties_img > ood_uncertainties_lidar, ood_uncertainties_img, ood_uncertainties_lidar)

    print(ind_uncertainties_fuse.shape, ood_uncertainties_fuse.shape)

    uncertainties = np.concatenate((ind_uncertainties_fuse, ood_uncertainties_fuse), axis=0)
    labels = np.concatenate((np.zeros_like(ind_uncertainties_fuse), np.ones_like(ood_uncertainties_fuse)), axis=0)
    ...
    # ind_uncertainties = (ind_uncertainties - np.min(uncertainties)) / (np.max(uncertainties) - np.min(uncertainties))
    ...
    fontsize = 15
    plt.hist([ind_uncertainties_fuse, ood_uncertainties_fuse], 20, 
            density=True, histtype="bar", alpha=1, color=['blue', 'red'], 
            label=['InD', 'OoD'])
    ...
    fontsize = 15
    point_size = 0.5
    plt.scatter(ind_uncertainties_lidar[::1000], ind_uncertainties_img[::1000], s=point_size)
    plt.scatter(ood_uncertainties_lidar[::100], ood_uncertainties_img[::100], s=point_size*5)
    # plt.legend(fontsize=fontsize)
    plt.xlabel(r"uncertainty LIDAR $\mu_0$", fontsize=fontsize)
    plt.ylabel(r'uncertainty Image $\mu_1$', fontsize=fontsize)
    plt.xticks(fontsize=fontsize)
    plt.yticks(fontsize=fontsize)
    # plt.xlim(0, 0.5e-5)
    # plt.ylim(0, 10.01)
    plt.tight_layout()
    plt.savefig(os.path.join(png_file, 'fuse_points_u.png'))
    plt.savefig(os.path.join(png_file, 'fuse_points_u.pdf'))
